...
// FIX: Import useAuth to read the guaranteed-ready user from context
import { useAuth } from '../../auth/context/AuthContext';

const CreateListingPage = () => {
  const navigate = useNavigate();
  // FIX: Get user from context — guaranteed non-null because AuthProvider
  // blocks render until auth resolves. No more race conditions.
  const { user } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    // FIX: Guard against unauthenticated submission clearly
    if (!user) {
    ...
    try {
      ...
      // Call Cloud Function — auth token is attached automatically by Firebase SDK
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '20px' }}>
      ...
      {error && (
        <div className="lux-card" style={{ background: 'rgba(220, 38, 38, 0.2)', color: '#fca5a5', padding: '1rem', marginBottom: '1rem' }}>
          {error}
        </div>
      )}
