...
const AdsXPage = () => {
  ...
  const loadRequests = useCallback(async () => {
    ...
    try {
      ...
      // FIX: Pass isPublic flag so the Cloud Function knows not to require auth
      // for this public browsing call (as opposed to 'my_requests' which needs auth)
      const result = await getRequests({
        limit: 20,
        serviceType: serviceType === 'All' ? null : serviceType,
        isPublic: true   // <-- tells the function: "public browse, no auth required"
      });

      setRequests(result.data.requests || []);
    } catch (err) {
      console.error('Fetch error:', err);
      // FIX: Show specific error messages for common failures
  ...
  return (
    <div className="min-h-screen w-full pr-6 text-white">
      ...
      {error && (
        <div className="lux-card bg-red-800/20 text-red-300 p-4 mb-6 flex items-center justify-between">
          <span>{error}</span>
          <button onClick={loadRequests} className="text-sm underline hover:text-white">Retry</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          <div className="text-muted animate-pulse col-span-3 text-center py-20">Loading requests...</div>
        ) : requests.length === 0 ? (
              ...
              <div>
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="text-xs text-muted font-mono">
                    {req.createdAt?._seconds
                      ? new Date(req.createdAt._seconds * 1000).toLocaleDateString()
                      : 'Recently'}
                  </span>
                </div>
